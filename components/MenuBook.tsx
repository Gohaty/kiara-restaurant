...
function PageFooter({
  ...
  return (
    <div
      className={`mt-auto flex items-center justify-between pt-9 font-smallcaps text-[10px] tracking-[0.45em] uppercase opacity-70 ${accent}`}
    >
      <span>{left}</span>
      <span>{right}</span>
    </div>
    ...
    >
      {hasStack ? (
        <div className="flex w-full flex-col gap-1">
          ...
          {item.description && (
            <p
              className={`mt-0.5 max-w-lg font-body text-[15px] leading-[1.5] tracking-[0.01em] italic ${descColor}`}
            >
              {item.description}
            </p>
          )}
